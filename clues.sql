...
-- Write SQL query here
SELECT countrylanguages.language, countries.name
FROM countries
JOIN countryLanguages ON countrylanguages.countrycode = countries.code
WHERE language = 'Italian' AND countrylanguages.percentage = 100;


-- Clue #4: We're booking the first flight out – maybe we've actually got a chance to catch her this time. There are only two cities she could be flying to in the country. One is named the same as the country – that would be too obvious. We're following our gut on this one; find out what other city in that country she might be flying to.

-- Write SQL query here
SELECT countrylanguages.language, cities.name
FROM cities
JOIN countryLanguages ON countrylanguages.countrycode = cities.countrycode
WHERE language = 'Italian' AND countrylanguages.percentage = 100 AND cities.name != 'San Marino';
